...
import { createSignatureRequest, getSignatureRequestStatus, cancelSignatureRequest, sendReminder } from '@/lib/signwell';
...
export async function POST(
  ...
  try {
    ...
    // Update document with signature request ID using admin client to bypass RLS
    ...
    if (updateError) {
      console.error('Error updating document with signature status:', updateError);
    } else {
      console.log('Document updated with signature status:', { id, signatureRequestId: result.signatureRequestId, status: 'pending', rows: updateData?.length });
    }
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to create signature request';
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
    );
